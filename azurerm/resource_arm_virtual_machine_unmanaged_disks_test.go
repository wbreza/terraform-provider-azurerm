...
func TestAccAzureRMVirtualMachine_changeOSDiskName(t *testing.T) {
	resourceName := "azurerm_virtual_machine.test"
	var afterCreate, afterUpdate compute.VirtualMachine

	ri := acctest.RandInt()
	preConfig := testAccAzureRMVirtualMachine_withNamedDisks(ri, testLocation(), "osDisk", "dataDisk")
	postConfig := testAccAzureRMVirtualMachine_withNamedDisks(ri, testLocation(), "osDiskRenamed", "dataDisk")
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "storage_os_disk.0.name", "osDisk"),
				),
			},

			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "storage_os_disk.0.name", "osDiskRenamed"),
					testAccCheckVirtualMachineRecreated(
						t, &afterCreate, &afterUpdate),
				),
			},
...
func TestAccAzureRMVirtualMachine_changeDataDiskName(t *testing.T) {
	resourceName := "azurerm_virtual_machine.test"
	var afterCreate, afterUpdate compute.VirtualMachine

	ri := acctest.RandInt()
	preConfig := testAccAzureRMVirtualMachine_withNamedDisks(ri, testLocation(), "osDisk", "dataDisk")
	postConfig := testAccAzureRMVirtualMachine_withNamedDisks(ri, testLocation(), "osDisk", "dataDiskRenamed")
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMVirtualMachineExists(resourceName, &afterCreate),
					resource.TestCheckResourceAttr(resourceName, "storage_data_disk.0.name", "dataDisk"),
				),
			},

			{
				...
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMVirtualMachineExists(resourceName, &afterUpdate),
					resource.TestCheckResourceAttr(resourceName, "storage_data_disk.0.name", "dataDiskRenamed"),
					testAccCheckVirtualMachineRecreated(
						t, &afterCreate, &afterUpdate),
				),
			},
...
//check to make sure adding a new data disk does not create a new resource WithoutRecreating
func TestAccAzureRMVirtualMachine_addNewDataDiskWithoutRecreating(t *testing.T) {
	resourceName := "azurerm_virtual_machine.test"
	var beforeAdd, afterAdd compute.VirtualMachine

	ri := acctest.RandInt()
	preConfig := testAccAzureRMVirtualMachine_withDataDisk(ri, testLocation())
	postConfig := testAccAzureRMVirtualMachine_withTwoDataDisks(ri, testLocation())
		...
		Steps: []resource.TestStep{
			{
				Config: preConfig,
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMVirtualMachineExists(resourceName, &beforeAdd),
					resource.TestCheckResourceAttr(resourceName, "storage_data_disk.0.name", "mydatadisk1"),
				),
			},

			{
				Config: postConfig,
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMVirtualMachineExists(resourceName, &afterAdd),
					resource.TestCheckResourceAttr(resourceName, "storage_data_disk.0.name", "mydatadisk1"),
					resource.TestCheckResourceAttr(resourceName, "storage_data_disk.1.name", "mydatadisk2"),
					func(s *terraform.State) error {
						if *beforeAdd.VMID != *afterAdd.VMID {
							t.Fatalf("Expected Virtual Machine IDs to not change, but before != after (%s != %s)", *beforeAdd.VMID, *afterAdd.VMID)
						}
						return nil
					},
				),
...
func testAccAzureRMVirtualMachine_withTwoDataDisks(rInt int, location string) string {
	return fmt.Sprintf(`
resource "azurerm_resource_group" "test" {
    name     = "acctestRG-%d"
    location = "%s"
...
func testAccAzureRMVirtualMachine_withNamedDisks(rInt int, location string, osDiskName string, dataDiskName string) string {
	return fmt.Sprintf(`
resource "azurerm_resource_group" "test" {
    name     = "acctestRG-%d"
    location = "%s"
...
`, rInt, location, rInt, rInt, rInt, rInt, rInt, osDiskName, dataDiskName, rInt)
}
